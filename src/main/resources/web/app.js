    ...
    function openModal(kind, data) {
      ...
      if (isRank) { syncPrefixPreview(); renderModalCommands(); } else renderUserModalCommands();
    ...
    function readRankEditor(){const id=document.getElementById('rank_id').value.trim().toLowerCase(); const meta=readJsonBox('rank_meta'); meta.gradientStart=document.getElementById('rank_grad_a')?.value || meta.gradientStart; meta.gradientMiddle=document.getElementById('rank_grad_m')?.value || meta.gradientMiddle; meta.gradientEnd=document.getElementById('rank_grad_b')?.value || meta.gradientEnd; return {name:id, displayName:document.getElementById('rank_name').value, prefix:document.getElementById('rank_prefix').value, weight:Number(document.getElementById('rank_weight').value||0), parents:splitList(document.getElementById('rank_parents').value), permissions:[...modalCommandRules(), ...readRules('rank')], meta};}
    function readUserEditor(original){return {uuid:document.getElementById('user_uuid').value, name:document.getElementById('user_name').value, primaryRank:document.getElementById('user_primary').value, ranks:splitList(document.getElementById('user_ranks').value), permissions:[...userModalCommandRules(), ...readRules('user')], meta:readJsonBox('user_meta'), lastSeen:original.lastSeen||Date.now(), online:!!original.online};}
    ...
    function newRank(){openModal('rank',{name:'nuevo',displayName:'Nuevo',prefix:'&7[Nuevo] ',weight:0,parents:[],permissions:[],meta:{gradientStart:'#4ff0ff',gradientMiddle:'#9c6bff',gradientEnd:'#ff4f9b'}})}
    ...
    function newUser(){const uuid=prompt('UUID del jugador offline'); if(!uuid)return; openModal('user',{uuid,name:'Offline',primaryRank:state.defaultRank||'default',ranks:[state.defaultRank||'default'],permissions:[],meta:{},online:false,lastSeen:Date.now()});}
